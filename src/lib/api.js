...
// Get video recommendation for a solved problem
export async function getVideoRecommendation(expression, topic) {
  try {
    const response = await fetch('http://localhost:3001/api/get-video-recommendation', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
      body: JSON.stringify({ expression, topic }),
    });
    ...
    throw new Error('Failed to get video recommendation. Please try again.');
  }
}
